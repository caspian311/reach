...
class CreateForeignKeyConstraints < ActiveRecord::Migration
   def self.up
      ...
      execute "alter table reach_player_medals add constraint fk_pm_medal foreign key (medal_id) references medals (id) on delete cascade"
   ...
   def self.down
      ...
      remove_index :weapons, :id
      remove_index :medals, :id
   end
end
